        ...
        DX.viz.charts.Axis = DX.Class.inherit(function() {
            ...
            var getTickValues = function() {
                    ...
                    if (self.overlappingBehavior) {
                        if (!options.isHorizontal && self.overlappingBehavior.mode !== 'ignore')
                            self.overlappingBehavior.mode = 'enlargeTickInterval';
            ...
            var adjustZoomValues = function(min, max) {
                    var that = this;
                    min = that.validateUnit(min);
                    max = that.validateUnit(max);
                    if (that.options.range && utils.isDefined(that.options.range.min)) {
                        min = utils.isDefined(min) ? that.options.range.min < min ? min : that.options.range.min : min;
                        max = utils.isDefined(max) ? that.options.range.min < max ? max : that.options.range.min : max
                    }
                    if (that.options.range && utils.isDefined(that.options.range.max)) {
                        max = utils.isDefined(max) ? that.options.range.max > max ? max : that.options.range.max : max;
                        min = utils.isDefined(min) ? that.options.range.max > min ? min : that.options.range.max : min
                    }
            ...
            var getRangeData = function() {
                    ...
                    if (options.isHorizontal) {
                        range.minX = min;
                        range.maxX = max;
                        range.minVisibleX = utils.isDefined(this.minRangeArg) ? this.minRangeArg : options.range.min;
                        range.maxVisibleX = utils.isDefined(this.maxRangeArg) ? this.maxRangeArg : options.range.max;
            ...
            var prototypeObject = {
                    ...
                    adjustZoomValues: adjustZoomValues
                };
        ...
        charts.BaseChart = ui.Component.inherit({
            ...
            _initRenderer: function _initRenderer() {
                ...
                _this.renderer = charts.factory.createRenderer({
                    ...
                    pathModified: _this.option('pathModified')
                })
            },
    ...
    (function($, DX, undefined) {
        ...
        charts.Chart = charts.BaseChart.inherit({
            ...
            _populateBusinessRange: function(visibleArea) {
                ...
                var argRange = new charts.Range({rotated: !!rotated});
            ...
            zoomArgument: function(min, max) {
                var that = this,
                    zoomArg,
                    rotated = that.option('rotated'),
                    axis = rotated ? that.verticalAxes[0] : that.horizontalAxes[0];
                zoomArg = axis.adjustZoomValues(min, max);
                    ...
                    $.each(axesInfo, function() {
                        ...
                        if (hasSyncronizedValue && this.tickInterval) {
                            ...
                            while ((lastTickValue = lastTickValue + this.tickInterval) <= this.maxValue)
                                this.tickValues.push(utils.isExponential(lastTickValue) ? utils.adjustValue(lastTickValue) : utils.applyPrecisionByMinDelta(this.minValue, this.tickInterval, lastTickValue))
                        }
