...
if (!DevExpress.MOD_VIZ_CORE) {
    ...
    (function($, DX) {
        ...
        core.__restoreLoadIndicator = function() {
            LoadIndicator = core._LoadIndicator
        }
    })(jQuery, DevExpress);
    ...
    (function($, DX) {
        DX.viz.core.widgetMarkupMixin = {
            ...
            _normalizeHtml: function(html) {
                ...
                return html = html.replace(/xmlns:NS1="[\s\S]*?"/gi, "").replace(/NS1:xmlns:xlink="([\s\S]*?)"/gi, 'xmlns:xlink="$1"')
            }
        }
    })(jQuery, DevExpress);
    ...
    (function($, DX, undefined) {
        DX.viz.core.registerTheme({
            ...
            chart: {
                ...
                commonSeriesSettings: {
                    label: {border: {color: '#4c4c4c'}},
                    candlestick: {innerColor: '#050506'}
                },
                commonAxisSettings: {
                    color: '#4c4c4c',
                    grid: {color: '#4c4c4c'},
    ...
    (function($, DX, undefined) {
        DX.viz.core.registerTheme({
            ...
            chart: {
                ...
                title: {font: {color: '#808080'}},
                commonSeriesSettings: {
                    label: {border: {color: '#b0b3ba'}},
                    candlestick: {innerColor: '#cbd0da'}
                },
                commonAxisSettings: {
                    color: '#b0b3ba',
                    grid: {color: '#b0b3ba'},
    ...
    (function($, DX) {
        DX.viz.core.registerTheme({
            ...
            chart: {
                ...
                title: {font: {color: '#ffffff'}},
                commonSeriesSettings: {
                    label: {border: {color: '#454545'}},
                    candlestick: {innerColor: '#000000'}
                },
                commonAxisSettings: {
                    color: '#454545',
                    grid: {color: '#454545'},
    ...
    (function($, DX) {
        ...
        function getPatternUrl(id, pathModified) {
            return 'url(' + (pathModified ? window.location.href : '') + '#' + id + ')'
        }
        ...
        var ShadowFilterSvgElement = BaseFilterSvgElement.inherit({
                _create: function() {
                    ...
                    _this.element.appendChild(finalComposite)
                },
        ...
        var createSingleRange = function(specificator) {
                ...
                rangeExtension['_applyValueMargins' + specificator] = function() {
                    ...
                    var applyMarginWithZeroCorrection = function(minSelector, maxSelector, rangeLength) {
                            var minValue = that[minSelector],
                                maxValue = that[maxSelector],
                            ...
                            if (isDefinedUtils(maxValue) && !maxCorrected && maxMargin)
                                that[maxSelector] = applyMargin(maxValue, maxMargin, rangeLength, 1, isDateTime);
                            if (isDefinedUtils(minValue) && !minCorrected && minMargin)
                                that[minSelector] = applyMargin(minValue, minMargin, rangeLength, -1, isDateTime)
                        };
                    ...
                    var processLogarithmicMinValue = function(valueField) {
                            if (isDefinedUtils(that[valueField])) {
                                ...
                                if (getLogUtils(that[valueField] / intermediateValue, base) < 0.1 && (that.keepValueMarginsX || that.keepValueMarginsY) && !that[keepValueMarginsSelector])
                                    that[valueField] = raiseToFlooredLog(that[valueField], base, -1);
                                else if ((that.keepValueMarginsX || that.keepValueMarginsY) && !that[keepValueMarginsSelector])
                    ...
                    var processLogarithmicMaxValue = function(valueField) {
                            if (isDefinedUtils(that[valueField])) {
                                ...
                                if (getLogUtils(that[valueField] / intermediateValue, base) > 0.5)
                                    that[valueField] = raiseToCeiledLog(that[valueField], base)
                            }
                        };
                    ...
                    if (that[axisTypeSelector] === 'logarithmic') {
                        ...
                        if (specificator === 'Y' && that.rotated === false || specificator === 'X' && that.rotated === true) {
                ...
                rangeExtension['_applyEqualLimitsMargins' + specificator] = function() {
                    var that = this,
                        isDateTime = isDateUtils(that[maxSelector]) || isDateUtils(that[minSelector]),
                        base = that[baseSelector],
                        isLogarithmic = that[axisTypeSelector] === 'logarithmic',
                        newMin,
                        newMax,
                        correction = isDateTime ? DATETIME_EQUALITY_CORRECTION : NUMBER_EQUALITY_CORRECTION;
                    if (isDefinedUtils(that[minSelector]) && isDefinedUtils(that[maxSelector]) && that[minSelector].valueOf() === that[maxSelector].valueOf()) {
                        newMin = that[minSelector].valueOf() - correction;
                        newMax = that[maxSelector].valueOf() + correction;
                        if (isLogarithmic) {
                            that[minSelector] = raiseToFlooredLog(that[minSelector], base, -correction);
                            that[maxSelector] = raiseToFlooredLog(that[maxSelector], base, correction)
                        }
                        else if (isDateTime) {
                            that[minSelector] = new Date(newMin);
                            that[maxSelector] = new Date(newMax)
                        }
                        else {
                            that[minSelector] = that[minSelector] === 0 ? that[minSelector] : newMin;
                            that[maxSelector] = newMax
                        }
                    }
                    if (isDefinedUtils(that[minVisibleSelector]) && isDefinedUtils(that[maxVisibleSelector]) && that[minVisibleSelector].valueOf() === that[maxVisibleSelector].valueOf()) {
                        newMin = that[minVisibleSelector].valueOf() - correction;
                        newMax = that[maxVisibleSelector].valueOf() + correction;
                        if (isLogarithmic) {
                            newMin = raiseToFlooredLog(that[minVisibleSelector], base, -correction);
                            newMax = raiseToFlooredLog(that[minVisibleSelector], base, correction);
                            that[minVisibleSelector] = newMin < that[minSelector] ? that[minSelector] : newMin;
                            that[maxVisibleSelector] = newMax > that[maxSelector] ? that[maxSelector] : newMax
                        }
                        else if (isDateTime) {
                            that[minVisibleSelector] = newMin < that[minSelector].valueOf() ? that[minSelector] : new Date(newMin);
                            that[maxVisibleSelector] = newMax > that[maxSelector].valueOf() ? that[maxSelector] : new Date(newMax)
                        }
                        else {
                            if (that[minVisibleSelector] !== 0)
                                that[minVisibleSelector] = newMin < that[minSelector] ? that[minSelector] : newMin;
                            that[maxVisibleSelector] = newMax > that[maxSelector] ? that[maxSelector] : newMax
                        }
                    }
    ...
    (function($, DX, undefined) {
        ...
        viz.charts.ThemeManager = viz.core.BaseThemeManager.inherit(function() {
            ...
            var applyNextSeriesTheme = function(userOptions, commonSeriesSettings, userCommonSeriesSettings) {
                    ...
                    mainSeriesColor = userOptions.color || mergedUserSettings.color || self.palette.getNextColor();
                    wrappedMainSeriesColor = new DX.Color(mainSeriesColor);
                    mergedSettings.color = mainSeriesColor;
                    mergedSettings.border.color = mergedSettings.border.color || mainSeriesColor;
                    mergedSettings.hoverStyle.color = mergedSettings.hoverStyle.color || self._IE8 && (isBar || isBubble || isArea) && wrappedMainSeriesColor.highlight(HOVER_COLOR_HIGHLIGHTING) || mainSeriesColor;
                    mergedSettings.hoverStyle.border.color = mergedSettings.hoverStyle.border.color || mainSeriesColor;
                    mergedSettings.selectionStyle.color = mergedSettings.selectionStyle.color || self._IE8 && (isBar || isBubble || isArea) && wrappedMainSeriesColor.highlight(HOVER_COLOR_HIGHLIGHTING) || mainSeriesColor;
                    mergedSettings.selectionStyle.border.color = mergedSettings.selectionStyle.border.color || mainSeriesColor;
                    mainPointColor = userOptions.point && userOptions.point.color || mergedUserSettings.point && mergedUserSettings.point.color || mainSeriesColor;
                    mergedSettings.point.color = mainPointColor;
                    mergedSettings.point.border.color = mergedSettings.point.border.color || isBar && mergedSettings.border.color || mainPointColor;
                    ...
                    mergedSettings.point.hoverStyle.border.color = mergedSettings.point.hoverStyle.border.color || isBar && mergedSettings.hoverStyle.border.color || mainPointColor;
                    ...
                    mergedSettings.point.selectionStyle.border.color = mergedSettings.point.selectionStyle.border.color || isBar && mergedSettings.selectionStyle.border.color || mainPointColor;
            ...
            var applyNextPieSegmentTheme = function applyNextPieSegmentTheme(userOptions, commonSeriesSettings) {
                    var self = this,
                        ...
                        wrappedMainColor;
                    userOptions = userOptions || {};
                    mainColor = userOptions.color || self.palette.getNextColor();
                    wrappedMainColor = new DX.Color(mainColor);
                    mergedSettings.color = mainColor;
                    mergedSettings.border.color = mergedSettings.border.color || mainColor;
                    mergedSettings.hoverStyle.color = mergedSettings.hoverStyle.color || self._IE8 && wrappedMainColor.highlight(HOVER_COLOR_HIGHLIGHTING) || mainColor;
                    mergedSettings.hoverStyle.border.color = mergedSettings.hoverStyle.border.color || mainColor;
                    mergedSettings.selectionStyle.color = mergedSettings.selectionStyle.color || self._IE8 && wrappedMainColor.highlight(HOVER_COLOR_HIGHLIGHTING) || mainColor;
                    mergedSettings.selectionStyle.border.color = mergedSettings.selectionStyle.border.color || mainColor;
                    return $.extend(true, {}, mergedSettings, userOptions)
                };
        ...
        series.RangeAreaSeries = series.BaseSeries.inherit({
            ...
            drawSeriesData: function(seriesElementsGroup, seriesMarkersGroup, labelsGroup, animationEnabled) {
                var self = this,
                    ...
                    topLineSegment,
                    bottomLineSegment,
                    borderVisible = self.styles.attributes.strokeWidth > 0 || self.styles.states.hover.strokeWidth > 0 || self.styles.states.selected.strokeWidth > 0;
                self.paths = [];
                self.areas = [];
                if (!self.segments)
                    return;
                ...
                $.each(self.segments, function(index, segment) {
                    ...
                    if (animationEnabled) {
                        areaSegment = self.getZeroAreaPoints(index);
                        bottomLineSegment = topLineSegment = self.getZeroPathPoints(index)
                    }
                    else {
                        areaSegment = self.areaSegments[index];
                        topLineSegment = segment;
                        bottomLineSegment = $.map(segment, function(pt) {
                            return pt.getCoords(true)
                        })
                    }
                    self.drawArea(seriesElementsGroup, index, areaSegment);
                    if (borderVisible) {
                        self.drawLine(seriesElementsGroup, topLineSegment);
                        self.drawLine(seriesElementsGroup, bottomLineSegment)
                    }
                });
                animationEnabled && self.animate(function() {
                    seriesMarkersGroup.animate(animationConst.showGroup, animationConst.showDuration);
            ...
            _animateBorders: function() {
                var self = this,
                    i,
                    segments = self.preparedSegments || self.segments,
                    bottomLine,
                    lineIndex;
                if (segments && self.paths && self.paths.length === segments.length * 2)
                    for (i = 0; i < segments.length; i++) {
                        lineIndex = i * 2;
                        self.paths[lineIndex].animate({points: segments[i]});
                        bottomLine = $.map(segments[i], function(pt) {
                            return pt.getCoords(true)
                        });
                        self.paths[lineIndex + 1].animate({points: bottomLine})
                    }
            },
            applyNormalStyle: function(mode) {
                this.applyPointNormalStyle(mode);
            ...
            animate: function(complete) {
                this.animateAreas(complete);
                this._animateBorders()
            }
        }).include(series.AreaDrawerMixin).include(series.SeriesPointsDrawerMixin).include(series.AreaSeriesAdjustOptionsMixin).include(series.pointVisualizationMixin).include(series.areaVisualizationMixin).include(series.pointsAnimation).include(series.areaAnimation).include(series.pathAnimation).include(series.pathVisualizationMixin).include(series.LineDrawerMixin).include(series.specialAreaMethodsMixin).include(series.RangeSeriesDataFilterMixin)
    })(jQuery, DevExpress);
