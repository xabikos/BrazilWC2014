...
if (!DevExpress.MOD_VIZ_GAUGES) {
        ...
        circularMarkers.TextCloudMarker = DX.viz.gauges.__internals.BaseTextCloudMarker.inherit({
            ...
            measure: function() {
                ...
                return {
                        ...
                        horizontalOffset: self._textFullWidth + (_Number(self._options.arrowLength) || 0),
                        verticalOffset: self._textFullHeight + (_Number(self._options.arrowLength) || 0)
                    }
            }
        })
    })(DevExpress);
        ...
        linearMarkers.TriangleMarker = DX.viz.gauges.__internals.linearNeedles.SimpleIndicator.inherit({
            _render: function() {
                ...
                if (self._vertical) {
                    ...
                    x2 = x1 + _Number(self._inverted ? options.length : -options.length);
                    settings.points = [x1, y1, x2, y1 - options.width / 2, x2, y1 + options.width / 2]
                }
                else {
                    y1 = options.y;
                    x1 = self._zeroPosition;
                    y2 = y1 + _Number(self._inverted ? options.length : -options.length);
        ...
        linearMarkers.TextCloudMarker = DX.viz.gauges.__internals.BaseTextCloudMarker.inherit({
            ...
            measure: function() {
                ...
                if (self._vertical) {
                    ...
                    if (self._inverted) {
                        minbound = options.x;
                        maxbound = options.x + arrowLength + self._textFullWidth
                    }
                    else {
                        minbound = options.x - arrowLength - self._textFullWidth;
                        maxbound = options.x
                    }
                }
                else {
                    ...
                    if (self._inverted) {
                        minbound = options.y;
                        maxbound = options.y + arrowLength + self._textFullHeight
                    }
                    else {
                        minbound = options.y - arrowLength - self._textFullHeight;
                        maxbound = options.y
                    }
                }
                return {
    ...
    (function(DX, undefined) {
        ...
        DX.viz.gauges.__internals.CircularRangeBar = DX.viz.gauges.__internals.BaseRangeBar.inherit({
            ...
            _setBarSides: function() {
                var self = this;
                self._maxSide = self._options.radius;
                self._minSide = self._maxSide - _Number(self._options.size)
            },
            _getSpace: function() {
                var options = this._options;
            ...
            _setTextItemsSides: function() {
                ...
                self._lineTo = self._lineFrom - _Number(options.text.indent);
                self._textRadius = options.radius + _Number(options.text.indent)
            },
            _getPositions: function() {
                ...
                return {
                        ...
                        main1: mainPosition1,
                        main2: mainPosition2,
                        back1: min(mainPosition1 + self._space, self._startPosition),
                        back2: max(mainPosition2 - self._space, self._endPosition)
                    }
            },
    ...
    (function(DX, undefined) {
        ...
        DX.viz.gauges.__internals.LinearRangeBar = DX.viz.gauges.__internals.BaseRangeBar.inherit({
            ...
            _setTextItemsSides: function() {
                var self = this,
                    indent = _Number(self._options.text.indent);
                if (indent > 0) {
            ...
            measure: function() {
                ...
                if (self._vertical) {
                    ...
                    if (self._hasText) {
                        ...
                        if (textIndent > 0)
                            maxbound += textIndent + self._textWidth;
                        if (textIndent < 0)
                            minbound += textIndent - self._textWidth
                    }
                }
                else {
                    minbound = maxbound = options.y;
                    ...
                    if (self._hasText) {
                        ...
                        if (textIndent > 0)
                            maxbound += textIndent + self._textHeight;
                        if (textIndent < 0)
                            minbound += textIndent - self._textHeight
                    }
                }
                return {
                        min: minbound,
        ...
        DX.viz.gauges.BarGauge = DX.viz.gauges.Gauge.inherit({
            ...
            _measureMainElements: function() {
                ...
                if (self._context.textEnabled) {
                    self._context.textColor = labelOptions && labelOptions.font && labelOptions.font.color || null;
                    ...
                    self._context.lineColor = labelOptions.connectorColor || null;
        ...
        _extend(BarWrapper.prototype, {
            ...
            arrange: function(options) {
                ...
                if (self._context.textEnabled) {
                    self._line.applySettings({
                        ...
                        stroke: self._context.lineColor || options.color
                    });
                    self._text.applySettings({font: {color: self._context.textColor || options.color}})
                }
                return self
            },
            getTooltipParameters: function() {
            ...
            setAngle: function(angle) {
                ...
                if (self._context.textEnabled) {
                    ...
                    self._text.applySettings({
                        text: _formatValue(self._value, self._context.formatOptions, {index: self._index}),
            ...
            beginAnimation: function(value) {
                var self = this;
                self._value = value;
                var angle = self._context.translator.translate(value);
                if (!compareFloats(self._angle, angle)) {
                    self._start = self._angle;
                    self._delta = angle - self._angle;
                    self._tracker.applySettings({visibility: 'hidden'});
                    if (self._context.textEnabled) {
            ...
            animate: function(pos) {
                var self = this;
                self._angle = self._start + self._delta * pos;
                setAngles(self._settings, self._context.baseAngle, self._angle);
                self._bar.applySettings(self._settings)
            },
            endAnimation: function() {
                var self = this;
                if (self._delta !== undefined) {
                    if (compareFloats(self._angle, self._start + self._delta)) {
                        self._tracker.applySettings({visibility: null});
                        if (self._context.textEnabled) {
